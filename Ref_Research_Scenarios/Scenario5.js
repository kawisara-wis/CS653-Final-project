...
const serviceConsumer = require('../services/Consumer_S5');
const serviceProvider = require('../services/Provider_S5');
const serviceOfferDirect = require('../services/OfferDirect_S5');
const serviceService = require('../services/Service_S5');
...
let numConsumers = 10; //sc2=25 sc3=5
...
mongoose.connect(config.db.url).then(async () => {
    ...
    try {
        ...
        for(let i = 0; i < numProviders - 1; i++){
            ...
            // (สุ่มพิกัด 0-100)
            let location = { x: Math.floor(Math.random() * 100), y: Math.floor(Math.random() * 100) };
            // (ส่ง 'location' เข้าไปใน 'options')
            providers.push(await serviceProvider.create(acc, { location: location }));
        }

        // สร้าง 1 AI Agent
        let accAI = await serviceAccount.create();
        // (สุ่มพิกัด 0-100)
        let locationAI = { x: Math.floor(Math.random() * 100), y: Math.floor(Math.random() * 100) };
        // (ส่ง 'location' เพิ่มเข้าไปใน 'options')
        let providerAI = await serviceProvider.create(accAI, { agentType: 'ai', location: locationAI });
        ...
        for(let i = 0; i < numConsumers; i++){
           // (สุ่มพิกัด 0-100)
            let location = { x: Math.floor(Math.random() * 100), y: Math.floor(Math.random() * 100) };
            // (ส่ง 'location' เข้าไปใน 'options')
            consumers.push(await serviceConsumer.create(await serviceAccount.create(), { location: location }));
        }
